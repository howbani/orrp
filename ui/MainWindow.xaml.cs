...
using ORR.Dataplane;
using ORR.db;
using ORR.Intilization;
using ORR.Coverage;
using ORR.Properties;
using System.Windows.Media;
using System.Windows.Threading;
using ORR.Forwarding;
using ORR.ExpermentsResults.Energy_consumptions;
using ORR.ExpermentsResults.Lifetime;
using ORR.ControlPlane.NOS.TC;
using ORR.ControlPlane.NOS.TC.subgrapgh;
using ORR.DataPlane.NeighborsDiscovery;
using ORR.ControlPlane.NOS.FlowEngin;
using ORR.ui.conts;
using ORR.Charts.Intilization;
using ORR.ControlPlane.NOS.Visualizating;
using System.Threading;
using ORR.Computations;
...
namespace ORR.ui
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public bool IsEaseSendPackets = false;
        ...
        private void TimerCounter_Tick(object sender, EventArgs e)
        {
            if (PublicParamerters.SimulationTime <= stopSimlationWhen + PublicParamerters.MacStartUp)
            {
                ...
                Dispatcher.Invoke(() => Title = "ORR:" + PublicParamerters.SimulationTime.ToString(), DispatcherPriority.Send);
                //re-compute ORR
                if (PublicParamerters.SimulationTime % PublicParamerters.UpdateORReveryXseconds == 0) 
                {
                    TimerCounter.Stop();
                    RandomSelectSourceNodesTimer.Stop();
                    //collet the data from the residual energy of nodes
                    ...
                    ORRComputing(PublicParamerters.SinkNode.ID);
                    //re-send the data from the sink to the node
                    foreach (Sensor sen in myNetWork)
                    {
                        if (sen.ID != PublicParamerters.SinkNode.ID)
                        {
                    ...
                    foreach (Sensor sen in myNetWork)
                    {
                        if (sen.ID != PublicParamerters.SinkNode.ID)
                        {
                            UplinkRouting.UpdateUplinkFlowEnery(sen);
                        }
                    }
                    TimerCounter.Start();
        ...
        public void ORRComputing(int sinkIndex) 
        {
            int rootNodeID = sinkIndex;
            PublicParamerters.SinkNode = myNetWork[rootNodeID];
            OrrNetwork orr = new OrrNetwork(myNetWork);
            orr.compute();
        }
        ...
        public void RandomDeplayment(int sinkIndex)
        {
            ...
            PublicParamerters.SizeofNetwork = myNetWork.Count;
            ORRComputing(sinkIndex);
        ...
        public void ClearExperment()
        {
            try
            {
                PublicParamerters.NumberofDatePackets = 0;
                PublicParamerters.NumberofDeliveredDataPackets = 0;
