...
namespace ORR.ControlPlane.NOS.FlowEngin
{
    ...
    public class UplinkRouting
    {
        ...
        {
            if (sender.ID != PublicParamerters.SinkNode.ID)
            {
                foreach (NeighborsTableEntry nei in sender.NeighborsTable)
                {
                    if (nei.NeiNode.ResidualEnergyPercentage >= 0)
                    {
                        MiniFlowTableEntry MiniEntry = new MiniFlowTableEntry();
                        MiniEntry.NeighborEntry = nei;
                        MiniEntry.NeighborEntry.EDC = nei.NeiNode.EDC;
                        if (sender.MyForwarders.Contains(nei.NeiNode))
                        {
                            MiniEntry.EDC = MiniEntry.NeighborEntry.EDC;
                ...
                {
                    MiniEntry.UpLinkPriority = MiniEntry.EDC;
                }
                ...
                foreach (MiniFlowTableEntry MiniEntry in sender.MiniFlowTable)
                {
                    ...
                    {
                        if (MiniEntry.NeighborEntry.NeiNode.ID == PublicParamerters.SinkNode.ID)
                        {
                            MiniEntry.UpLinkAction = FlowAction.Forward;
                        }
                        else
